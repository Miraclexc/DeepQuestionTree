...
class Pruner:
    ...
    async def summarize_path(self, leaf_node: Node, session: SessionData) -> str:
        ...
        try:
            ...
            for node in path_nodes:
                if node.interaction:
                    qa = {
                        ...
                        "answer": node.interaction.summary
                        or node.interaction.answer[:100] + "...",
                    }
                    path_qas.append(qa)
            ...
            prompt = self.prompts.render(
                "summarize_pruned_path", qa_text=qa_text, context_note=context_note
            )
            # 调用 LLM 生成摘要
    ...
    def _get_path_to_root(self, node: Node, session: SessionData) -> List[Node]:
        """
        获取从节点到根节点的路径
        """
        path: list[Node] = []
    ...
    def prune_subtree(
        self, root_node_id: str, reason: str, session: SessionData
    ) -> int:
        """
        剪枝子树
        ...
        logger.info(
            f"剪枝子树 {root_node_id}，共剪枝 {pruned_count} 个节点，原因：{reason}"
        )
    ...
    def get_pruning_statistics(self, session: SessionData) -> Dict[str, Any]:
        ...
        for node in session.nodes.values():
            if node.is_pruned and node.prune_reason:
                prune_reasons[node.prune_reason] = (
                    prune_reasons.get(node.prune_reason, 0) + 1
                )
        ...
        stats = {
            ...
            "pruned_percentage": (
                (pruned_nodes / total_nodes * 100) if total_nodes > 0 else 0
            ),
